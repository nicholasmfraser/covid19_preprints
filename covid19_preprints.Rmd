...
sample_date_until <- as.Date("2021-12-27")
...
##TO DO: include (not: replace) query for types.resourceTypeGeneral:Preprint as per DataCite metadata schema 4.4 


##specify years to include
dc_years <- c("2020", "2021")
#dc_years <- "2021" # for iterative update, only need to use 2021
...
dc_expected_results <- map2(dc_years, dc_types, getDataCiteCount) %>%
  map("total") %>%
  map_int(sum) %>%
  sum()
...
dc_preprints <- map2(dc_years, dc_types, getDataCitePreprints) %>%
  purrr::flatten()
...
repec_covid <- getRepecPreprints(sample_date_from,
                                 sample_date_until) %>%
  bind_rows() %>% 
