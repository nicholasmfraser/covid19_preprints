...
Note: run update from previous sample date (inclusive) to ensure no records are missed due to time zone differences on day of sampling. Deduplicate final dataset to filter out records that are sampled twice.

```{r message = FALSE, warning = FALSE}
#set sample_date_until
sample_date_until <- "2020-12-20"

sample_date_from <- fromJSON(
...
sample_date_from <- sample_date_from %>%
  .$sample_date 
#NB use date in character format for API queries
...
'Created date' is used for querying updates, so as not to miss any new entries with an older 'posted date'. Also, entries with a 'posted date' after the sample date are not filtered out, as these will not be retrieved in subsequent updated when using 'created date'. 
...
cr_expected_results <- cr_types(types = "posted-content",
                                works = TRUE,
                                limit = 1,
                                filter = c(from_created_date = sample_date_from, 
                                           until_created_date = sample_date_until)
                                )$meta$total_results
...
cr_posted_content_covid <- cr_posted_content_df %>%
  ...
  # "true" preprints
  filter(!is.na(source)) %>%
  # Filter on preprints posted after 2020-01-01
...
cr_ssrn_expected_results <- cr_works(
  filter = c(issn = "1556-5068",
             from_created_date = sample_date_from,
             until_created_date = sample_date_until),
...
cr_ssrn_covid <- cr_ssrn_df %>%  
  ...
  mutate(posted_date = ymd(map_chr(identifier, getSSRNPublicationDate)),
         source = "SSRN") %>%
  #filter on preprints with posted date since 2020
  filter((posted_date >= "2020-01-01")) %>%
   # Select only relevant fields with unique values
  distinct(source, identifier, identifier_type, posted_date, title, abstract)
...
Results can be filtered on date of creation (year only), which works fine for 2020 (but will need adaptation afterwards).This means that it's not possible to only query results from last sample date onwards. Pagination is used to get all records.
...
ar_covid_new <- arxiv_search('ti:coronavirus OR ti:covid OR ti:sars-cov OR ti:ncov-2019 ti:2019-ncov OR ti:hcov-19 OR ti:sars-2 OR abs:coronavirus OR abs:covid OR abs:sars-cov OR abs:ncov-2019 OR abs:2019-ncov OR abs:hcov-19 OR abs:sars-2', limit = 10000) %>%
  mutate(source = "arXiv",
         identifier = id,
         identifier_type = "arXiv ID",
         posted_date = date(submitted)) %>%
  # Filter preprints submitted in sampling period
  filter((posted_date >= sample_date_from) & (posted_date <= sample_date_until)) %>%
  # Select only relevant fields with unique values
  distinct(source, identifier, identifier_type, posted_date, title, abstract)
...
# Create combined dataset ((bind Crossref, DataCite, arXiv and RePEc data)
...
#NB DO not remove preprints with posted_date after sample date, as these will (for Crossref) not be retrieved anew in subsequent updates.
...
# Combine with previous dataset, deduplicate preprint versions
...
covid_preprints <- covid_preprints %>%
  # Some preprints have been sampled twice due to inclusive sample dates
  distinct() %>%
...
write_csv(covid_preprints, "data/covid19_preprints.csv")
...
covid_preprints %>%
  ...
  labs(x = "Posted Date", y = "Preprints", fill = "Source",
       ...
       subtitle = paste0("(up until ", sample_date_until, ")"),
