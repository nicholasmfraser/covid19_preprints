...
getDataCiteCount <- function(year){
...
dc_expected_results <- map(dc_years, getDataCiteCount) %>%
  map("total") %>%
  map_int(sum)
  
##define function to execute Datacite iteratively
getDataCitePreprints <- function(year){
  ...
  for (i in seq){
     ...
     str_remove(paste0("https://api.datacite.org/dois\\?created=",
                      year,
                      "&page%5Bcursor%5D=")) %>%
     str_remove("&page%5Bsize%5D=1000&query=types.resourceType%3APreprint")
...
dc_preprints <- map(dc_years, getDataCitePreprints) %>%
  purrr::flatten()
...
ar_covid0 <- arxiv_search('ti:coronavirus OR ti:covid OR ti:sars-cov OR ti:ncov-2019 ti:2019-ncov OR ti:hcov-19 OR ti:sars-2 OR abs:coronavirus OR abs:covid OR abs:sars-cov OR abs:ncov-2019 OR abs:2019-ncov OR abs:hcov-19 OR abs:sars-2', limit = 10000) %>% 
  mutate(source = "arXiv",
         identifier = id,
...
covid_preprints_update <- bind_rows(cr_covid, dc_covid, ar_covid, repec_covid) %>%
  select(source, identifier, identifier_type, posted_date, title, abstract) 
...
# Minimum number of preprints to be included in graphs (otherwise too many categories/labels is confusing)
n_min <- 100
