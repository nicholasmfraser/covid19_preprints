...
Preprint data is currently updated on a weekly schedule by collecting preprints added since the previous sample date. Details of the current release can be found in `data/metadata.json`, where `sample_date` refers to the date on which data was collected, and `posted_date` the cut-off point for preprints to be included in the accompanying visualizations based on their posting date.
