...
parsePreprints2 <- function(x){
  x %>% mutate(
    title = map(title, "title", .null = NA),
    description = map(descriptions, "description", .null = NA),
    descriptionType = map(descriptions, "descriptionType", .null = NA)) %>%
    select(-descriptions)
}
#add unnest(title) to parsePreprints2 creates additional rows
unnest(title)



#only keep title field from original list element 'titles'
#some records have 2 elements in title field - collapse into 1
#NB do this after descriptions, add description as grouping variable!
...
data_df2 <- data_df_title %>%
  left_join(data_df_abstract, by = c("IDseq", "doi", "created", "client")) %>%
  distinct()


extract2 <- map(extract, parsePreprints2)
extract3 <- parsePreprints2(extract)
extract3_title <- parseTitles(extract)
extract3_des <- parseDescriptions(extract)
